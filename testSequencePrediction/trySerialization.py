...
def processSequence(model, iteration, sequence):
  for position in sequence:
    modelCopy = pickle.loads(pickle.dumps(model))
    modelCopy._deSerializeExtraData("/home/docker/testSequencePrediction/foo")
    processPosition(model, iteration, position)
    processPosition(modelCopy, iteration, position)
